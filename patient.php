<?php 	session_start();
	...
	{		
		...
		{
			$id=$_POST['edited'];
			$query="update patient set pname='$pname',age='$age',height='$height',weight='$weight',gender='$gender',email='$email' where pid=$id";
		}
		else
		{
			$query="insert into patient (pname,age,height,weight,gender,email) values('$pname','$age','$height','$weight','$gender','$email')";
		}
		...
		else
		{
			?>
			<script>
			alert ("Patient record not added");
			</script>
			<?php
		}
	}
					...
					<form class="form" method="POST">
						<input type="hidden" name="edited" value="<?php if(!empty($row['pid'])) echo $row['pid']; ?>"/>
					     <table class="doctor">
							...
							<tr>	
								<td>Enter Email:</td>
								<td><input type="email" name="email" required value="<?php if(!empty($row['email'])) echo $row['email']; ?>"/> <td>
							</tr>
							<tr>
